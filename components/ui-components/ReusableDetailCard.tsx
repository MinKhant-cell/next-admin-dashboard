...
import { ReactNode, createContext } from 'react'; // ✅ Add createContext
...
interface ReusableDetailCardProps {
  title: string;
  backHref: string;
  children: ReactNode;
  isLoading?: boolean;
...
export function ReusableDetailCard({
  title,
  backHref,
  children,
  isLoading,
      ...
      <Card className={`w-full p-5 sm:overflow-auto mb-5 ${className}`}>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-3 sm:gap-0">
          ...
          <div className="sm:flex-shrink-0 sm:mr-5 self-start">
            <BreadCrumbs 
              className="block sm:inline underline sm:no-underline hover:underline text-sm sm:text-base" 
